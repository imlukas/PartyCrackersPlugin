package dev.imlukas.partycrackersplugin.util.command.language;
...
import dev.imlukas.partycrackersplugin.util.command.language.parser.argument.ParameterArgument;
...
public abstract class AbstractObjectiveModel<T extends CompiledObjective> {
    ...
    private void addCompletions(List<String> suggestions, String lastWord, ParameterArgument<?> parameterArgument) {
        ...
        if (type instanceof FilteredParameterType<?> filtered) {
            Collection<String> completions = filtered.getSuggestions();

            for (String filteredValue : completions) {
                if (filteredValue.startsWith(lastWord)) {
                    suggestions.add(filteredValue);
                }
            }
        }
    }
